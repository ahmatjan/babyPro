...
#ifdef ITTDEBUG
    #define NSLog(format, ...) NSLog((@"%s [Line %d] " format), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);  //打印函数名，行数，内容
#else
    #define NSLog(format, ...)  //什么都不打印
#endif

// 和NSLog功能差不多，分开解耦
#ifdef ITTDEBUG
    #define ITTDPRINT(xx, ...)  NSLog( xx, ##__VA_ARGS__)
#else
